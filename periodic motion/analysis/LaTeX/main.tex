...
\begin{equation}
    ...
    \label{sp1}
\end{equation}
where the angular frequency is $\omega_0=\sqrt{g/l}$ and period $T=2\pi/\omega_0$.
...
\begin{equation}
    \ddot{\theta} + \gamma \dot{\theta} + \omega_0^2 \sin{\theta} = A_d \cos{\omega_d t}
    \label{sp3}
\end{equation}

where,
\begin{itemize}
    \item $\gamma  = \dfrac{b}{ml^2}$, is the damping factor,
    \item $\omega_d$ is the driving angular frequency, and
...
For the simulation the following constant parameters were set:

\begin{enumerate}
    \item Mass of the pendulum ($m$): \SI{1}{\kilogram}
    \item Length of pendulum ($l$): \SI{3}{\meter}
    \item Damping coefficient ($b$): \SI{0.05}{\kilogram\per\second}
    \item Driving torque amplitude ($\tau_d$): \SI{1}{\newton\meter}
    \item Driving frequency ($\omega_d$): \SI{1.8}{\radian\per\second}
    \item Natural frequency ($\omega_0$): \SI{1.8}{\radian\per\second}
\end{enumerate}
...
\begin{itemize}
    ...
    \item The amplitude and width of these beats decrease over time.
...
Analysis of the following system parameters: amplitude, period, frequency, and angular frequency shows that:
\begin{itemize}
...
\begin{itemize}
    \item At the start of the motion (0 to 100s), the pendulum attains its absolute maximum velocity ($\approx$ \SI{2}{\radian\per\second}) and displacement ($\approx$ \SI{1}{\radian}).
